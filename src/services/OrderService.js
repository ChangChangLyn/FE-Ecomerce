...
export const AccessOrder = async (id, data) => {
  const res = await axiosJWT.put(
    `${process.env.REACT_APP_API_URL}/order/access-order/${id}`,
    data
  );
...
export const revenueMonthly = async (access_token) => {
  const res = await axiosJWT.get(
    `${process.env.REACT_APP_API_URL}/order/revenue-monthly`,
    {
      headers: {
        token: `Bearer ${access_token}`,
      },
