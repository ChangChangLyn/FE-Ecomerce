...
export const getProductsByCategory = async (categoryId) => {
  const res = await axios.get(
    `${process.env.REACT_APP_API_URL}/product/products-by-category/${categoryId}`
  );
  return res.data;
};
...
export const createProductReview = async (id, data) => {
  const res = await axios.post(
    `${process.env.REACT_APP_API_URL}/product/reviews/${id}`,
    data
  );
  return res.data;
};
export const repReviewProduct = async (id, access_token, data) => {
  const res = await axios.post(
    `${process.env.REACT_APP_API_URL}/product/rep-reviews/${id}`,
    data,
    {
      headers: { token: `Bearer ${access_token}` },
    }
